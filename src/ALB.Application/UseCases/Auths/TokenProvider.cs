...
public sealed class TokenProvider(IRefreshTokenRepository refreshTokenRepository, IOptions<JwtOptions> options)
{
    ...
    public string CreateInviteToken(Guid id, string email)
    {
        var signingKey = new SymmetricSecurityKey(
            Encoding.UTF8.GetBytes(options.Value.Secret));
        var credentials = new SigningCredentials(signingKey, SecurityAlgorithms.HmacSha256);
        ...
        claims.Add(new Claim(ClaimTypes.Role, SystemRoles.Invited));
